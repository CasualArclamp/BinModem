...
//! comes down is read by [`super::pcm`] and interpreted by
//! [`super::downstream`] -- Jd and J'd from the signs, the route from the DIL,
//! R from its sign pattern, and TRN2d, MP, Ed, B1d and data from whole data
//! frames. What is here is the state machine that drives those, and decides
//! what goes up in answer to each.
...
use crate::v34::mp::{Found, Mp, Trellis};
...
use super::digital;
use super::downstream::{DilReader, Frames, JdReader, RSeen, RWatch, find_place, least_gap, levels_for, nearest, slicer_for};
use super::encoder::{Decoder, Frame, Mapping};
use super::pcm::{self, Heard, Slicer};
use super::sequences::{self, Cp, Descriptor, JD_BITS, Jd};
...
/// looking. How often after that, how much is kept to look in and how much of
/// it a start is judged on are [`super::downstream`]'s.
...
impl Modem {
    ...
    pub fn new(settings: Settings, fs: f64) -> Self {
        ...
        let mut modem = Self {
            ...
            source,
            rx,
            dil: DilReader::new(&descriptor, settings.law, settings.uinfo),
            descriptor,
            jd: JdReader::new(),
            far_jd: None,
            jd_gone: false,
    ...
    fn heard(&mut self, heard: Heard) {
        match heard {
            ...
            Heard::Lost if self.stage == Stage::Dil && !self.dil.is_lost() => {
    ...
    fn symbol(&mut self, symbol: pcm::Symbol) {
        ...
        match self.stage {
            Stage::AwaitJd | Stage::AwaitJdPrime => {
                // V.90 has nothing but a Jd to read here, so every frame that
                // frames is offered to `Jd::from_bits`; V.92's analogue modem
                // is the one with a Jp to refuse.
                let jd_prime = self.jd.feed(symbol.index, symbol.positive(), |_| true);
                ...
                self.dil.keep(symbol.raw, symbol.value);
                ...
                if gone
                    && self.dil.ready_to_look()
                    && symbol.raw.is_multiple_of(64)
                    && let Some(start) = self.dil.find_start()
                {
                    self.begin_dil(start.first, &start.already);
                }
            }
            Stage::Dil => {
                if self.dil.symbol(&mut self.rx, symbol.raw, symbol.value) {
                    self.finish_dil();
                }
            }
    ...
    fn begin_dil(&mut self, first: u64, already: &[(u64, f64)]) {
        ...
        if self.dil.begin(&mut self.rx, first, already) {
            self.finish_dil();
        }
    }
    ...
    fn phase4_symbol(&mut self, symbol: pcm::Symbol) {
        ...
        frames.keep(symbol.index, symbol.value);
        if i != INTERVALS - 1 {
            return;
        }
        let frame = Frame {
        ...
        frames.note(!looked && !frames.decoder.could_have_sent(&frame));
        if frames.lost_place()
